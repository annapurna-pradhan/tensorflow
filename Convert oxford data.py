...
from scipy.misc import imread, imsave 
 ...
 tf.app.flags.DEFINE_string(     'dataset_dir', 'data/datasets',     'The location of Oxford IIIT Pet Dataset which contains         annotations and images folders'     )     
 tf.app.flags.DEFINE_string(     'target_dir', 'data/train_data',     'The location where all the images will be stored'     )      
     ...
     if split_name not in ["trainval", "test"]:     
         ...
         for data in tqdm(list_data, desc=split_name):       
             ...
             if image is not None:       
                 target_class_dir =         
                 ensure_folder_exists(os.path.join(target_split_path,           class_name))       
